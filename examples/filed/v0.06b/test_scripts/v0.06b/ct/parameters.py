...
class Calc(AbstractParameter):
    ...
    def _conditional_tasmax_duration_(cls, kwargs):
        ...
        if rd.variable == 'tasmax':
            ret = [{'func': 'duration', 'name': 'duration',
                    'kwds': {'threshold': 21.0, 'operation': 'gte', 'summary': 'mean'}}]
        return (ret)
        ...
        for filename in filenames:
            ...
            for time in [time_range, time_region]:
                if time is None:
                    ...
                    trange = None
                    tregion = time
                rd = ocgis.RequestDataset(os.path.join(path, filename), variable, time_range=trange,
                                          time_region=tregion)
                yield (rd)
