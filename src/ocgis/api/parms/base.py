...
class OcgParameter(object):
    ...
    def _set_value_(self, value):
        ...
        if not any(type_matches):
            raise (DefinitionValidationError(self,
                                             'Input value type "{1}" is not in accepted types: {0}'.format(input_types,
                                                                                                           type(
                                                                                                               value))))
    ...
    def parse(self, value):
        """:rtype: varies depending on `ocgis.api.parms.base.OcgParameter` subclass"""
            ...
            for idx in range(len(ret)):
                try:
                    ...
                    if type(ret[idx]) not in self.element_type:
                        raise (DefinitionValidationError(self,
                                                         'Element type incorrect. Acceptable types are: {0}'.format(
                                                             self.element_type)))
                    else:
                        pass
                except ValueError:
                    raise (DefinitionValidationError(self, 'Element type incorrect. Acceptable types are: {0}'.format(
                        self.element_type)))
