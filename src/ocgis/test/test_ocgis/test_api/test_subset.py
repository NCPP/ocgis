...
class TestSubsetOperation(TestBase):
    ...
    def test_bounding_box_wrapped(self):
        ...
        bbox = [-104, 36, -95, 44]
        rd_global = self.test_data_nc.get_rd('cancm4_tas')
    ...
    def test_regridding_same_field(self):
        ...
        rd_dest = self.test_data_nc.get_rd('cancm4_tas')
        ...
        for ctr, k in enumerate(itr_products_keywords(keywords, as_namedtuple=True)):
            ...
            rd2 = self.test_data_nc.get_rd('cancm4_tas', kwds={'alias': 'tas2'})
            ...
            if isinstance(k.regrid_destination, SpatialDimension):
                ...
                rd1 = self.test_data_nc.get_rd('cancm4_tas', kwds={'crs': Spherical()})
                rd2 = self.test_data_nc.get_rd('cancm4_tas', kwds={'alias': 'tas2', 'crs': Spherical()})
    ...
    def test_regridding_different_fields_requiring_wrapping(self):
        ...
        rd1 = self.test_data_nc.get_rd('cancm4_tas')
        rd2 = self.test_data_nc.get_rd('maurer_2010_tas')
    ...
    def test_regridding_different_fields_variable_regrid_targets(self):
        ...
        rd1 = self.test_data_nc.get_rd('cancm4_tas')
        rd2 = self.test_data_nc.get_rd('maurer_2010_tas', kwds={'time_region': {'year': [1990], 'month': [2]}})
    ...
    def test_regridding_update_crs(self):
        ...
        for k in itr_products_keywords(keywords, as_namedtuple=True):
            if k.assign_source_crs:
                ...
                rd1 = self.test_data_nc.get_rd('narccap_lambert_conformal', kwds={'crs': rd1.crs})
            else:
                rd1 = self.test_data_nc.get_rd('narccap_lambert_conformal')

            if k.assign_destination_crs:
                # assign the coordinate system changes some regridding behavior. in this case the value read from file
                # is the same as the assigned value.
                rd2 = self.test_data_nc.get_rd('cancm4_tas', kwds={'crs': CFWGS84()})
            else:
        ...
        # test regridding lambert conformal to 0 to 360 grid
        rd1 = self.test_data_nc.get_rd('cancm4_tas')
        rd2 = self.test_data_nc.get_rd('narccap_lambert_conformal')
    ...
    def test_regridding_two_projected_coordinate_systems(self):
        ...
        rd1 = self.test_data_nc.get_rd('narccap_lambert_conformal')
        rd2 = self.test_data_nc.get_rd('narccap_polar_stereographic')
