...
def compute(ops,tile_dimension,verbose=False,use_optimizations=True):
    '''
    :param :class:~`ocgis.OcgOperations`: The target operations to tile.
    :param int tile_dimension: The target tile/chunk dimension.
    :param bool verbose: If `True`, print more verbose information to terminal.
    :param bool use_optimizations: If `True`, reuse :class:`Field` and :class:`TemporalGroupDimension`
    ...
    try:
        ...
        if ops.geom is not None:
            if verbose: print('geometry subset is present. calculating slice offsets...')
            ...
            for row in coll.get_iter_melted():
                ...
                assert(row['variable']._value == None)
